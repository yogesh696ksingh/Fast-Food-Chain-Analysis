...
<nav class="navbar navbar-expand-lg navbar-light bg-light" style="margin-bottom: 2em">
    <ul class="nav nav-pills">
        <li class="nav-item">
            <a class="nav-link" href="/mapDashboard">Heat Map</a>
        </li>
        <li class="nav-item">
            <a class="nav-link active" href="/detailDashboard">Detail Dashboard</a>
        </li>
        <li class="nav-item">
...
<div class="container">
    ...
    <div class="row">
        <div class="col-sm-6 offset-4">
            <form method="POST" action="/viewDetailData" class="form-inline" name="stateSelectForm">
                {{-- <div class="form-group mb-2">
                    <label>Select State</label>
                </div> --}}
                <div class="form-group mx-sm-3 mb-2">
                    <select class="form-control" id="stateSelect" name="restaurant" required>
                        @if (Request::isMethod('post'))
                        <option selected value="{{ $selectedrestaurant }}">{{ $selectedrestaurant }}</option>
                ...
                <div class="form-group mx-sm-3 mb-2">
                        <select class="form-control" id="stateSelect" name="state" required>
                            @if (Request::isMethod('post'))
                            <option selected value="{{ $selectedstate }}">{{ $selectedstate }}</option>
    ...
    <?php if($_SERVER['REQUEST_METHOD'] == 'POST') { ?>
        ...
        <div id="pieChartPos" class="chart"></div>
    </div>
    <?php } ?>
</div>
  ...
  $("#pieChartPos").drawPieChart([
      <?php 
        foreach($stats as $key => $value ) {
            ...
            echo '{ title: "'.$key.'",         value : '.$value.',  color: "rgb('.$r.', '.$g.', '.$b.')" },';
        }
      ?>
    { title: "End",         value : 0,    color: "#04374E" }
  ]);
});
/*!
